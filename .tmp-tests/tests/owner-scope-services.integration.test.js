...
describe("owner-scoped services", () => {
    ...
    it("enforces owner-scoped reads for products", async () => {
        ...
        expect(mockDb.getAllAsync).toHaveBeenCalledWith(expect.stringContaining("WHERE owner_id = ?"), 11);
    });
    it("does not expose shopper pin data in read models", async () => {
        ...
        if (result.ok) {
            ...
            expect(result.value[0].pin).toBeUndefined();
        }
    });
    ...
    it("returns conflict error for duplicate barcode", async () => {
        mockDb.runAsync.mockRejectedValueOnce(new Error("UNIQUE constraint failed: product.owner_id, product.barcode"));
    ...
    it("stores derived shopper pin payload instead of plaintext pin digits", async () => {
        ...
        expect(result.ok).toBe(true);
        expect(mockDb.runAsync).toHaveBeenCalledWith(expect.stringContaining("pin_hash, pin_key, pin"), 11, "Shopper A", expect.stringContaining("scrypt$"), expect.stringMatching(/^[0-9a-f]+$/), expect.any(Number), expect.any(Number));
    ...
    it("returns conflict error for duplicate shopper pin across different owners", async () => {
        mockDb.runAsync.mockRejectedValueOnce(new Error("UNIQUE constraint failed: shopper.pin_key"));
        const result = await shopperService.createShopper({
            name: "Shopper A",
            pin: "1234",
        });
    ...
    it("updates shopper name without clearing pin hash when pin is omitted", async () => {
        ...
        expect(result.ok).toBe(true);
        expect(mockDb.runAsync).toHaveBeenCalledWith(expect.not.stringContaining("pin_hash"), "Renamed Shopper", expect.any(Number), 51, 11);
    });

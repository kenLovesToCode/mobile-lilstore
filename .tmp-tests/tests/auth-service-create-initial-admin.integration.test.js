...
jest.mock("@/domain/services/entry-gate-runtime", () => ({
    updateEntryGateSnapshotAfterAdminChange: (...args) => mockUpdateEntryGateSnapshotAfterAdminChange(...args),
}));
describe("createInitialMasterAdmin", () => {
    ...
    it("creates the first admin record with normalized username and derived credential material", async () => {
        const result = await (0, auth_service_1.createInitialMasterAdmin)({
            username: "  MasterUser  ",
            password: "Password123!",
            nowMs: 1700000000000,
        });
        expect(mockBootstrapDatabase).toHaveBeenCalled();
        expect(mockDb.getFirstAsync).toHaveBeenCalledWith(expect.stringContaining("SELECT COUNT(*) as admin_count"));
        expect(mockDb.withExclusiveTransactionAsync).toHaveBeenCalled();
        expect(mockTxn.runAsync).toHaveBeenCalledWith(expect.stringContaining("INSERT INTO admin"), "masteruser", expect.stringMatching(/^scrypt\$N=\d+\$r=\d+\$p=\d+\$dkLen=\d+\$salt=.*\$hash=.*/), 1700000000000, 1700000000000);
        ...
        expect(mockUpdateEntryGateSnapshotAfterAdminChange).toHaveBeenCalledWith(true);
    ...
    it("returns safe error when transaction fails", async () => {
        mockDb.withExclusiveTransactionAsync.mockRejectedValueOnce(new Error("database is locked"));
        const result = await (0, auth_service_1.createInitialMasterAdmin)({
            username: "owner",
            password: "Password123!",
        });
        ...
        expect(mockWarn).toHaveBeenCalledWith("[auth-service] createInitialMasterAdmin failed", { reason: "Error" });
    ...
    it("normalizes admin usernames using trim + lowercase", () => {
        expect((0, auth_service_1.normalizeAdminUsername)("  MasterUser  ")).toBe("masteruser");
    });
});

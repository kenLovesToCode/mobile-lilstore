...
const mockSerializeDerivedPasswordCredential = jest.fn(
  (_value: unknown) => "scrypt$mock-dummy-material",
);
...
jest.mock("@/domain/services/password-derivation", () => ({
  ...
  derivePasswordCredentialMaterial: (...args: unknown[]) =>
    mockDerivePasswordCredentialMaterial(...args),
  serializeDerivedPasswordCredential: (value: unknown) =>
    mockSerializeDerivedPasswordCredential(value),
  verifyPasswordCredentialMaterial: (...args: unknown[]) =>
    mockVerifyPasswordCredentialMaterial(...args),
}));

let authenticateAdmin: typeof import("@/domain/services/auth-service").authenticateAdmin;
...
describe("authenticateAdmin", () => {
  beforeEach(() => {
    ...
    mockDerivePasswordCredentialMaterial.mockResolvedValue({
      storageValue:
        "scrypt$N=16384$r=8$p=1$dkLen=32$salt=abcd$hash=1234",
    });
    mockDb.getFirstAsync.mockResolvedValue({
      id: 7,
      username: "masteruser",
      password_hash:
        "scrypt$N=16384$r=8$p=1$dkLen=32$salt=0011$hash=2233",
    });
  ...
  it("authenticates a valid admin using normalized username + password verification", async () => {
    ...
    expect(mockDb.getFirstAsync).toHaveBeenCalledWith(
      expect.stringContaining("FROM admin"),
      "masteruser",
    );
    expect(mockVerifyPasswordCredentialMaterial).toHaveBeenCalledWith(
      "Password123!",
      "scrypt$N=16384$r=8$p=1$dkLen=32$salt=0011$hash=2233",
    );
  ...
  it("returns invalid-credentials when credential material is malformed", async () => {
    mockVerifyPasswordCredentialMaterial.mockRejectedValueOnce(
      new Error("Invalid stored credential format."),
    );
    ...
    expect(mockWarn).toHaveBeenCalledWith(
      "[auth-service] authenticateAdmin failed",
      { reason: "CredentialVerificationError" },
    );
  });

  it("returns a safe service error when verification fails for runtime reasons", async () => {
    mockVerifyPasswordCredentialMaterial.mockRejectedValueOnce(
      new Error("TextEncoder is unavailable in this runtime."),
    );
    ...
    expect(mockWarn).toHaveBeenCalledWith(
      "[auth-service] authenticateAdmin failed",
      { reason: "CredentialVerificationRuntimeError" },
    );
  });
  ...
  it("refreshes stored credentials in the background when params are stale", async () => {
    ...
    expect(mockDerivePasswordCredentialMaterial).toHaveBeenCalledWith(
      "Password123!",
    );
    await Promise.resolve();
    expect(mockDb.runAsync).toHaveBeenCalledWith(
      expect.stringContaining("UPDATE admin"),
      "scrypt$N=16384$r=8$p=1$dkLen=32$salt=abcd$hash=1234",
      expect.any(Number),
      7,
    );
